	...
	if (SizeOfStack(h) < size_stack) {
		...
		if (x + 2 <= 8 && y + 1 <= 8 && a[x + 2][y + 1] == 0) {
			...
			if (h->inf_x == coord_finish.x && h->inf_y == coord_finish.y) {
				...
				cout << "ВЕРШИНА КОТОРУЮ МЫ ОБОЗНАЧАЕМ ЕДИНИЧКОЙ = " << h->inf_x << " " << h->inf_y << endl;
				a[h->inf_x][h->inf_y] = 1;
		...
		if (x - 1 >= 1 && y - 2 >= 1 && a[x - 1][y - 2] == 0) {
			...
			if (h->inf_x == coord_finish.x && h->inf_y == coord_finish.y) {
				...
				a[h->inf_x][h->inf_y] = 1;
				push(h, x_y_head.x, x_y_head.y);
				cout << "Мы дошли до конца" << endl << endl;
			...
			cout << "------Дальше конь ходить не может!" << endl << endl;
			cout << "------------------------------------------------------------------------" << endl;
			cout << "Единички превращаем обратно в нули!" << endl;
			if (x + 2 <= 8 && y + 1 <= 8) {
				a[x + 2][y + 1] = 0;
			}
			if (x - 2 >= 1 && y + 1 <= 8) {
