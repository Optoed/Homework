...
void push(list *&h, list *&t, int x) {		//вставка элемента в 
	list* r = new list;						//создаем новый элемент
	r->inf = x;								//информационное поле inf = x
	r->next = NULL;							//всегда последний(?)

	if (!h && !t) {							//если двусвязный список пуста
		r->prev = NULL;							//это и голова, и хвост
		h = r;
	}

	else {
		t->next = r;						//r - следующий для хвоста
		r->prev = t;								//теперь r - хвост
	}
...
void del_node(list*& h, list*& t, list* r) {				//удаление элемента из очереди
	if (r == h && r == t) {
		h = t = NULL;
	}
	else if (r == h) {
...
int main() {							//Главная функция
	...
	for (int i = 0; i < n; i++) {		//Создаем стек
		cin >> x;						//Вводим число
		push(head, tail, x);					//Заполняем стек
	}
