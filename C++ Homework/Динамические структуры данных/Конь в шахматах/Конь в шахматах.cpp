	...
	if (SizeOfStack(h) < size_stack) {
		...
		if (x + 2 <= 8 && y + 1 <= 8 && a[x + 2][y + 1] == 0) {
			...
			if (h->inf_x == coord_finish.x && h->inf_y == coord_finish.y) {
				...
				//Записываем этот стек h куда-нибудь и запоминаем его длину и отмечаем как минимум и size_stack приравниваем его размеру (готово)
				size_stack = SizeOfStack(h);
				//А теперь надо удалить 2 верхушки (если не можем, то одну) и вызвать рекурсивно функцию move - ГОТОВО
				pop(h);
				pop(h);
		...
		if (x - 2 >= 1 && y + 1 <= 8 && a[x - 2][y + 1] == 0) {
			...
			if (h->inf_x == coord_finish.x && h->inf_y == coord_finish.y) {
				...
				push(h, x_y_head.x, x_y_head.y);
				cout << "Мы дошли до конца" << endl;
				//Записываем этот стек h куда-нибудь и запоминаем его длину и отмечаем как минимум, а потом size_stack приравниваем его размеру, и вызываем рекурсивную функцию снова

				//А теперь надо удалить 2 верхушки (если не можем, то одну) и вызвать рекурсивно функцию move
				return h;
			}
			else {
				coordinates x_y_head = pop(h);
